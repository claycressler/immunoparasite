...
pars <- c(r1=0.1, r2=0.1, K1=100, K2=100, a12=2, a21=2)
...
## Take these simulations and plot the configuration of the nullclines and how they change as the parasite and immune response grow
...
```{r grant_figure, fig.width=12, fig.height=9, units='in', res=450, fig.cap="Figure for grant proposal."}
params = c(S1=1000, S2=1000, s1=2000, s2=2000, b1=0.1, b2=0.1, I12=10000, I21=10000, m=0.9, c1=100, c2=130, C1=50, C2=50, bp=4, Kp=300, a=0.004)
params2 <- c(params, T1=800)
eq1 <- findEquilibrium(T2P_cline_model, y0=c(1200,0), parameters=params2, system="two.dim", plot.it=FALSE, summary=FALSE)
eq2 <- findEquilibrium(T2P_cline_model, y0=c(200,200), parameters=params2, system="two.dim", plot.it=FALSE, summary=FALSE)
eq3 <- findEquilibrium(T2P_cline_model, y0=c(700,0), parameters=params2, system="two.dim", plot.it=FALSE, summary=FALSE)
eq4 <- findEquilibrium(T2P_cline_model, y0=c(500,200), parameters=params2, system="two.dim", plot.it=FALSE, summary=FALSE)
...
legend(x='topleft', legend=c("Th2 nullclines", "P nullclines"), cex=1.4, lwd=3, col=c(1,2), bty='n')
text(x=2000, y=450, "A", cex=1.5, adj=0)
mtext(side=3, line=0.5, "Dynamical Profile of\nImmune-Parasite Interaction", cex=1)
points(eq1$ystar[1,1], eq1$ystar[2,1], pch=21, bg=1, cex=2.5)
points(eq2$ystar[1,1], eq2$ystar[2,1], pch=21, bg=1, cex=2.5)
points(eq3$ystar[1,1], eq3$ystar[2,1], cex=2.5)
points(eq4$ystar[1,1], eq4$ystar[2,1], cex=2.5)
...
library(animation)
if(!file.exists("movie1_for_mathbio_talk.mp4")) {
...
```{r grant_figure_4, fig.height=3.5, fig.width=7.5, units='in', fig.cap="Dose is 60 across all simulations. The first contrast shows a shift towards a chronic infection caused by weakening the negative feedbacks, as might be expected if the system shifts towards tolerance. The second shows a shift towards a chronic infection caused by strengthening the baseline level of Th1 stimulation, thereby increasing the initial Th1ness of the system."}
...
par(mfrow=c(1,3), mar=c(2.5,5,1,1), oma=c(2.5,0,0,0))
plot(out1[,c(1,4)], type='l', lwd=3, lty=2, col=gray(0.8), xlab='', ylab='', ylim=c(0,300), cex.axis=1.5)
mtext(side=2, line=3.25, "Parasite biomass", cex=1.5)
mtext(side=1, line=0.25, outer=T, "Days post-infection", cex=1.5)
#lines(out2[,c(1,4)], lwd=3, col=gray(0.6))
lines(out3[,c(1,4)], lwd=3, col=gray(0.4))
lines(out4[,c(1,4)], lwd=3, col=gray(0))
legend(x='topright', c("Baseline","Increase Th1ness","Weaken clearance"), fill=c(gray(1),gray(0.4),gray(0)), bty='n', cex=1.2)

plot(out1[,c(1,3)], type='l', lwd=3, lty=2, col=gray(0.8), xlab='', ylab='', ylim=c(0,1600), cex.axis=1.5)
mtext(side=2, line=3.25, "Th2 response", cex=1.5)
...
mtext(side=2, line=3.25, "Th1 response", cex=1.5)
