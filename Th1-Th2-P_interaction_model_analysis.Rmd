...
## Start Th1 biased
out10.3 <- ode(y=c(T1=800,T2=500,P=10), times=seq(0,500,0.1), full_model, params)
out20.3 <- ode(y=c(T1=800,T2=500,P=20), times=seq(0,50,0.1), full_model, params)
out40.3 <- ode(y=c(T1=800,T2=500,P=40), times=seq(0,50,0.1), full_model, params)
out80.3 <- ode(y=c(T1=800,T2=500,P=80), times=seq(0,50,0.1), full_model, params)
...
d <- 120
out1 <- ode(y=c(T1=800,T2=500,P=d), times=seq(0,50,0.1), full_model, params)
plot(out1[,c(1,3)], type='l')
...
## 120 but spread out - still clearance
d <- 30
eventdat <- data.frame(var=c("P"),
                       time=c(5,10,15),
                       value=c(d),
                       method=c("add"))
out2 <- ode(y=c(T1=800,T2=500,P=d), times=seq(0,50,0.05), full_model, params, events=list(data=eventdat))
plot(out2[,c(1,4)], type='l')

## spread further and no clearance
d <- 30
eventdat <- data.frame(var=c("P"),
                       time=c(10,20,30),
                       value=c(d),
...
## If dose is really small, even if dosing happens very frequently, won't get clearance, even if the total dose ends up being larger than the initial bolus
d <- 20
eventdat <- data.frame(var=c("P"),
                       time=1:10,
                       value=c(d),
...
## Everything happens so fast in this model. Need for things to slooowwww down.
## Strong chronicity-promoting feedbacks
params = c(S1=1000, S2=1000, s1=2000, s2=2000, b1=0.1, b2=0.1, I12=10000, I21=10000, m=0.9, c1=50, c2=130, C1=20, C2=20, bp=0.03, Kp=300, a=0.0001)

## Dose of 60 is cleared
out1 <- ode(y=c(T1=800,T2=500,P=60), times=seq(0,500,0.1), full_model, params)
plot(out1[,c(1,4)], type='l')
...
eventdat <- data.frame(var=c("P"),
                       ...
                       value=c(d),
                       method=c("add"))
out2 <- ode(y=c(T1=800,T2=500,P=d), times=seq(0,500,0.1), full_model, params, events=list(data=eventdat))
plot(out2[,c(1,4)], type='l')

## In fact, can even add more if spread far enough
d <- 20
eventdat <- data.frame(var=c("P"),
                       time=c(4,8,12,16),
                       value=c(d),
...
## If time is short enough, the extra will change the outcome
d <- 20
eventdat <- data.frame(var=c("P"),
                       time=c(0.5,1),
                       value=c(d),
...
tail(out80.3 <- ode(y=c(T1=800,T2=500,P=10), times=seq(0,50,0.1), full_model, params))
