...
// [[Rcpp::export]]
List nested_modelC2(NumericVector params) {
//int nested_model(NumericVector params) {
  ...
  while(t < tmax && I > 0) {
    ...
    prod1 = ifelse(P > 0.0, b1 + c1*P/(C1+P) + s1*pow(T1,2.0)/(pow(S1,2.0)+pow(T1,2.0)) * I12/(I12+T2), b1);
    prod2 = ifelse(P > 0.0, b2 + c2*P/(C2+P) + s2*pow(T2,2.0)/(pow(S2,2.0)+pow(T2,2.0)) * I21/(I21+T1), b2);
    decay1 = m*T1;
    decay2 = m*T2;
    ...
    // Parasite death
      ...
      if (Hosts(ind,2) < 1.0) {
        ...
        I -= 1;
        R += 1;
        Rprintf("parasite extinction S %i\n",S);
        Rprintf("I %i\n",I);
