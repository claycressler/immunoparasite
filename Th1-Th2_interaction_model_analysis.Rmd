...
Let's consider this model for the immune dynamics, then:
\begin{equation}
\frac{dT_i}{dt} = b_i + c_i P + \frac{s_i T_i^2}{S_i^2 + T_i^2}\frac{I_{ij}}{I_{ij}+T_j} - m T_i.
\end{equation}
We can compare what the configurations of the isoclines look like for different values of $b_i$, $c_i$, and $P$ to see how parasite dynamics will shape the trajectories through the phase plane.
When parasites are absent $P=0$, there should only be one stable equilibrium (the resting state; Fig. \@ref(fig:P0clines)).

```{r P0clines, echo=FALSE, fig.height=6, fig.width=8, fig.cap="Null clines and equilibria when the parasite is at its initial value and the parameters are (S1=1000, S2=1000, s1=2000, s2=2000, b1=0.1, b2=0.1, I12=10000, I21=10000, m=1, c1=1, c2=1, P=0)."}
derivs = function(t, y, params) {
  s1 <- params["s1"]
  s2 <- params["s2"]
  b1 <- params["b1"]
...
if(!file.exists("movie1.mp4"))
  saveVideo({ 
    ...
    for (i in seq(0,250,10)) {
      params = c(S1=1000, S2=1000, s1=2000, s2=2000, b1=0.1, b2=0.1, I12=10000, I21=10000, m=1, c1=1, c2=1, P=i)
      ...
      clines <- nullclines(derivs, 
                           ...
                           lwd=2,
                           state.names=c("Th1","Th2"))
    }}, video.name="movie1.mp4", other.opts = "-pix_fmt yuv420p -b 300k")
...
What I'm going to do now is a stochastic simulation where the parasite abundance grows logistically, with no effect on it by the immune response, just to observe the dynamics.
When the parasite reaches its equilibrium, the phase plane and isoclines are exactly as they are shown in Fig. \@ref(fig:Pclines1), so the question I am asking here is whether stochastic variation is enough to tip the system towards a polarized response on its way towards the co-stimulation equilibrium.
In this first simulation, I will assume that the parasite stimulates both immune responses equally well, and that the system starts in a state where the only stable equilibrium is the low co-activation equilibrium.
```{r Pdyn, eval=FALSE, fig.height=6, fig.width=8, fig.cap="Fifty replicate stochastic trajectories of Th1 and Th2 responses as a parasite population grows towards carrying capacity, assuming no effect of the immune response on parasite growth, that the parasite stimulates both Th1 and Th2 equally well, and that the immune response starts out balanced."}
gillespie_sim <- function(tmax, y0, params, seed=NULL) {
  if (!is.null(seed))
    set.seed(seed)
  s1 <- params["s1"]
  ...
  while (t < tmax) {
    ## compute event rates
    prod1 <- b1 + s1*T1^2/(S1^2+T1^2) * I12/(I12+T2) + c1*P
    prod2 <- b2 + s2*T2^2/(S2^2+T2^2) * I21/(I21+T1) + c2*P
    death1 <- m*T1
    death2 <- m*T2
    birthP <- bp*P*(1-P/Kp)
...
params = c(S1=1000, S2=1000, s1=2000, s2=2000, b1=0.1, b2=0.1, I12=10000, I21=10000, m=1, c1=1, c2=1, P=10, bp=1, Kp=200)

library(magrittr)
seeds <- runif(50, 1, 1000) %>% floor

lapply(seeds,
       function(s) gillespie_sim(50, y0=c(5, 5), params=params, seed=s)
) -> sims

par(mfrow=c(1,2), mar=c(3,3,0,0), oma=rep(0.5,4))
plot.new()
plot.window(xlim=c(0,100), ylim=c(0,400))
axis(1); axis(2); box('plot')
for (i in 1:length(seeds))
  lines(sims[[i]][,1], sims[[i]][,2], col=gray(0.4))
mtext(side=1, line=2, "Time")
mtext(side=2, line=2, "Th1 response")

plot.new()
plot.window(xlim=c(0,100), ylim=c(0,400))
axis(1); axis(2); box('plot')
for (i in 1:length(seeds))
  lines(sims[[i]][,1], sims[[i]][,3], col=gray(0.4))
mtext(side=1, line=2, "Time")
mtext(side=2, line=2, "Th2 response")

```



```{r Pdyn2, eval=FALSE, fig.height=6, fig.width=8, fig.cap="Fifty replicate stochastic trajectories of Th1 and Th2 responses as a parasite population grows towards carrying capacity, assuming no effect of the immune response on parasite growth and that the parasite stimulates both Th1 and Th2 equally well, but that the immune response starts out biased towards Th1"}
params = c(S1=1000, S2=1000, s1=2000, s2=2000, b1=0.1, b2=0.1, I12=10000, I21=10000, m=1, c1=1, c2=1, P=10, bp=1, Kp=200)
seeds <- runif(50, 1, 1000) %>% floor

if(!file.exists("gillespieSims1.RDS")) {
  lapply(seeds,
         function(s) gillespie_sim(100, y0=c(9, 1), params=params, seed=s)
  ) -> sims
  saveRDS(sims, "gillespieSims1.RDS")
} else sims <- readRDS("gillespieSims1.RDS")

par(mfrow=c(1,2), mar=c(3,3,0,0), oma=rep(0.5,4))
plot.new()
plot.window(xlim=c(0,100), ylim=c(0,1500))
axis(1); axis(2); box('plot')
for (i in 1:length(seeds))
  lines(sims[[i]][,1], sims[[i]][,2], col=gray(0.4))
mtext(side=1, line=2, "Time")
